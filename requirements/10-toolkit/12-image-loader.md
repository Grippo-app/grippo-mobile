...
The reference repo wraps `AsyncImage` per design-system component rather than inlining it at call sites — see `:design-system:components/example/ExerciseExampleImage.kt`:
...
- **Sizes/radii live in a component-scoped `AppDp` group** (`AppTokens.dp.exerciseExampleImage.medium.size`). The `AppDp.icon` / `AppDp.padding` / `AppDp.size` / `AppDp.radius` primitives are `private` inside `AppDp` — call sites must use a public component group (`exerciseExampleImage`, `userCard`, `equipmentCard`, …). Add a new public group if no existing one fits.
- **Placeholders/errors come from `AppTokens.icons.<Name>`** (ImageVector extensions in `:design-system:resources:provider/icons/`), wrapped in a small `Painter` adapter (`rememberInsetVectorPainter`). Photographic placeholders ship as bare-noun `.webp` under `composeResources/drawable/` (e.g. `barbell.webp`, `dumbbell.webp`) and are reached via `AppTokens.drawables.res(Res.drawable.<name>)` — there is no `img_*` prefix convention.
