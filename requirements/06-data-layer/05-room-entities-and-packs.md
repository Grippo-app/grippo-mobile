...
3. **Mutations are `suspend`.** Use `@Insert(onConflict = OnConflictStrategy.REPLACE)` per child entity (e.g. `insertTraining`, `insertExercises`, `insertIterations`) plus a `@Transaction suspend fun insertOrReplace(parent, children, grandchildren)` default-method that fans out the inserts in one DB transaction. `@Update` is **not** used — every mutation is an upsert via `REPLACE`.
4. **`@Insert(onConflict = OnConflictStrategy.REPLACE)`** — upsert semantics. Server is the source of truth; if it sends a record with an existing ID, overwrite.
