...
// Cheap, poll-safe currency signals for the component comparison projection.
// Reads exactly two small project-owned files (adapter config + component
// mapping registry) per call and keeps an in-memory project-dirty latch fed
// by durable invalidation receipts and the startup reconciler. It never
// hashes repository sources on a GET (REQ-PERF-002): exact input
// fingerprints are re-taken only by plan/start/publish and by the startup
// pass.
...
var MAPPING_FILE = path.join(paths.PROJECT_ROOT, 'orchestrator', 'figma', 'component-mappings.json');
...
// Adapter config presence signal for the components capability. Full
// semantic validation belongs to the trusted runner; the projection only
// distinguishes the three states whose next action differs (configure / fix
// file / proceed).
function adaptersSignal() {
  ...
  var enabled = document.adapters.filter(function (adapter) {
    return adapter && adapter.enabled === true &&
      Array.isArray(adapter.capabilities) && adapter.capabilities.indexOf('components') >= 0;
  }).length;
  var domainHash;
  try { domainHash = adapterConfigIdentity.capabilityHash(document, 'components'); }
...
function mappingSignal() {
  ...
  if (!document || document.schemaVersion !== 2 || !Number.isSafeInteger(document.revision) || document.revision < 0) {
    return { state: 'invalid', revision: null, fileHash: sha(read.bytes) };
  }
  return { state: 'present', revision: document.revision, fileHash: sha(read.bytes) };
}

function readComponentSignals() {
...
module.exports = {
  ...
  readComponentSignals: readComponentSignals,
